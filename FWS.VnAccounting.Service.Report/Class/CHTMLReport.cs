...
namespace FWS.VnAccounting.Service.Report.Class
{
    public class CHTMLReport
    {
        ...
        private string SetReportData(string reportBody, DataSet datasource)
        {
            ...
            while (findIndex > 0)
            {
                ...
                if (findIndex < endIndex)
                {
                    ...
                    string map = reportBody.Substring(findIndex, endIndex - findIndex-1);
                    int tableNumber = -1;
                    if (map.Contains('.')) int.TryParse(map.Substring(0, map.IndexOf('.')), out tableNumber);
                    else tableNumber = 0;
                    string mapField = map.Substring(map.IndexOf('.'));
        ...
        private byte[] ExportToPdf(string html, bool isUsePageNumber = true)
        {
            ...
            pProcess.StartInfo.Arguments = arg;// "--webpage --quiet " + sFontSize + m_sWaterMark + " --bodyfont Arial " + sLandScape + " -t pdf14 -f " + sFileName + ".pdf " + sFileName + ".html";
