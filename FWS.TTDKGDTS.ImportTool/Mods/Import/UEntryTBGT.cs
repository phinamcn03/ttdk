...
{
    public partial class UEntryTBGT : UBase
    {
        #region Contructor
        public UEntryTBGT()
        {
        ...
        */
        ...
        public bool ValidateInput()
        {
            ...
            {
                if (string.IsNullOrEmpty(txtObjectEmail.Text.Trim()))
                {
                    Message = "Phải nhập email CSGT trước.";
                    txtObjectEmail.Focus();
                    return false;
                }

                //if (string.IsNullOrEmpty(txtWarranterEmail.Text.Trim()))
                //{
                //    Message = "Email khách hàng không hợp lệ";
                //    txtWarranterEmail.Focus();
                //    return false;
                //}
            }

            if (chkSendEmailKH.Checked)
            {
                if (string.IsNullOrEmpty(txtWarranterEmail.Text.Trim()))
                {
                    Message = "Phải nhập email khách hàng";
                    txtObjectEmail.Focus();
                    return false;
                }
            }
        ...
        private void txtVoucherActionType_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtVoucherActionType.SelectedValue != null && txtVoucherActionType.SelectedValue.Equals("94"))
                {
                    ...
                    //Hien cot C.T va ModifyNote
                    if (grAssetOther.Columns.Contains("colIsModify") && grAssetOther.Columns.Contains("colModifyNote"))
                    {
                        grAssetOther.Columns["colIsModify"].Visible = true;
                        grAssetOther.Columns["colModifyNote"].Visible = true;
                    }
                    if (grAsset.Columns.Contains("colIsModify") && grAsset.Columns.Contains("colModifyNote"))
                    {
                        grAsset.Columns["colIsModify"].Visible = true;
                        grAsset.Columns["colModifyNote"].Visible = true;
                    }
                }
                else
                {
                    ...
                    //An cot C.T va ModifyNote
                    if (grAssetOther.Columns.Contains("colIsModify") && grAssetOther.Columns.Contains("colModifyNote"))
                    {
                        grAssetOther.Columns["colIsModify"].Visible = false;
                        grAssetOther.Columns["colModifyNote"].Visible = false;
                    }

                    //An cot C.T va ModifyNote
                    if (grAsset.Columns.Contains("colIsModify") && grAsset.Columns.Contains("colModifyNote"))
                    {
                        grAsset.Columns["colIsModify"].Visible = false;
                        grAsset.Columns["colModifyNote"].Visible = false;
                    }
